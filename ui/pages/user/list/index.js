...
Page({
  data: { items: [], loading: true, formOpen:false, form:{ name:'', role:'base_agent', base_ids:[], password:'' }, bases:[], checkedBaseIds:{}, roleRange:['admin','base_agent','captain','factory_manager'], roleIndex:1, saving:false, fabStyle:'' },
  async onShow(){
    ...
    try{
      ...
      this.setData({ items: mapped, bases });
  ...
  onRole(e){ const i=Number(e.detail.value); this.setData({ roleIndex:i, 'form.role': this.data.roleRange[i] }); },
  ...
  async onEdit(e){ const id = e.currentTarget.dataset.id; const curRaw=this.data.items.find(x=>x.id===id); if(!curRaw) return; try{ const detail=await req.get('/api/user/get?id='+id); const base_ids=(detail.bases||[]).map(b=>b.id); const checked={}; base_ids.forEach(i=>checked[i]=true); const idx=this.data.roleRange.indexOf(detail.role||'base_agent'); this.setData({ formOpen:true, form:{ id:detail.id, name:detail.name, role:detail.role, base_ids:base_ids, password:'' }, checkedBaseIds:checked, roleIndex: idx>=0?idx:1 }); }catch(err){ wx.showToast({ title:'加载详情失败', icon:'none' }); } },
