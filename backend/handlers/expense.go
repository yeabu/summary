...
func CreateExpenseBatch(w http.ResponseWriter, r *http.Request) {
    ...
    if role != "admin" && role != "base_agent" && role != "captain" { http.Error(w, "无权创建开支记录", http.StatusForbidden); return }
    ...
    if role == "base_agent" || role == "captain" {
        ...
        if len(codes) > 0 {
            ...
            if err := tx.Where("code IN ?", codes).Find(&bs).Error; err == nil {
                for _, b := range bs { allowedBaseIDs = append(allowedBaseIDs, b.ID) }
            }
        }
        if len(allowedBaseIDs) == 0 {
            tx.Rollback(); http.Error(w, "当前用户未绑定基地", http.StatusBadRequest); return
        }
    }
    ...
    for _, req := range items {
        ...
        if role == "base_agent" || role == "captain" {
            ...
            // 若都未提供则默认第一个允许基地
            var target uint
            if req.BaseID != 0 { target = req.BaseID } else if payload.BaseID != 0 { target = payload.BaseID }
            if target != 0 {
                ok := false
                for _, id := range allowedBaseIDs { if id == target { ok = true; break } }
                if !ok { failed++; continue }
                bid := target; baseID = &bid
            } else {
                bid := allowedBaseIDs[0]; baseID = &bid
            }
        } else { // admin
            if req.BaseID != 0 {
        ...
        exp := models.BaseExpense{
            ...
            Amount:     req.Amount,
            Currency:   func() string { if req.Currency != "" { return req.Currency }; return "CNY" }(),
            Detail:     req.Detail,
...
func CreateExpense(w http.ResponseWriter, r *http.Request) {
    ...
    if role == "admin" {
        ...
        if req.BaseID != 0 {
            ok := false
            for _, b := range bs { if b.ID == req.BaseID { ok = true; break } }
            if !ok { http.Error(w, "无权在该基地创建开支", http.StatusForbidden); return }
            baseID = req.BaseID
        } else {
            baseID = bs[0].ID
        }
...
func ListExpense(w http.ResponseWriter, r *http.Request) {
    ...
    if role := claims["role"].(string); role == "base_agent" || role == "captain" {
        ...
        if len(codes) > 0 {
            ...
            if err := db.DB.Where("code IN ?", codes).Find(&bs).Error; err == nil {
                var ids []uint
                for _, b := range bs { ids = append(ids, b.ID) }
                if len(ids) > 0 { query = query.Where("base_id IN ?", ids) }
            }
        }
    } else if role == "admin" {
    ...
    if len(missingIDs) > 0 {
        ...
        for id := range missingIDs { ids = append(ids, id) }
        ...
        if err := db.DB.Select("id, name").Where("id IN ?", ids).Find(&users).Error; err == nil {
            ...
            for i := range expenses {
                if expenses[i].CreatorName == "" {
                    if n, ok := nameMap[expenses[i].CreatedBy]; ok { expenses[i].CreatorName = n }
                }
            }
        }
    }
...
func UpdateExpense(w http.ResponseWriter, r *http.Request) {
    ...
    db.DB.Model(&item).Updates(models.BaseExpense{
        ...
        CategoryID: req.CategoryID,
        Amount:     req.Amount,
        Currency:   func() string { if req.Currency != "" { return req.Currency }; return item.Currency }(),
        Detail:     req.Detail,
        UpdatedAt:  time.Now(),
    })
	db.DB.Preload("Base").Preload("Category").First(&item, eid)
...
func StatExpense(w http.ResponseWriter, r *http.Request) {
    ...
    if role := claims["role"].(string); role == "base_agent" || role == "captain" {
        ...
        if len(codes) > 0 { group = group.Where("bases.code IN ?", codes) } else { group = group.Where("1=0") }
    } else {
