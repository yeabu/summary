...
func GetProductPurchaseParam(w http.ResponseWriter, r *http.Request) {
	...
	if pid <= 0 {
		http.Error(w, "product_id 无效", http.StatusBadRequest)
		return
	}
...
func UpsertProductPurchaseParam(w http.ResponseWriter, r *http.Request) {
	...
	if role, _ := claims["role"].(string); role != "admin" && role != "base_agent" && role != "warehouse_admin" {
	...
	if err := db.DB.Where("product_id = ?", req.ProductID).First(&cur).Error; err == nil {
		...
		if e := db.DB.First(&prod, req.ProductID).Error; e == nil && prod.Currency != "" {
			cur.Currency = prod.Currency
		}
	...
	cur = models.ProductPurchaseParam{ProductID: req.ProductID, Unit: strings.TrimSpace(req.Unit), FactorToBase: req.FactorToBase, PurchasePrice: req.PurchasePrice, Currency: func() string {
		if prod.Currency != "" {
...
func DeleteProductPurchaseParam(w http.ResponseWriter, r *http.Request) {
	...
	if err := db.DB.Where("product_id = ?", pid).Delete(&models.ProductPurchaseParam{}).Error; err != nil {
