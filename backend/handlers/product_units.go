...
func ListProductUnitSpecs(w http.ResponseWriter, r *http.Request) {
    ...
    if pid == 0 { http.Error(w, "product_id 必填", http.StatusBadRequest); return }
...
func UpsertProductUnitSpec(w http.ResponseWriter, r *http.Request) {
    ...
    if body.ProductID == 0 || body.Unit == "" || body.FactorToBase <= 0 { http.Error(w, "参数不完整", http.StatusBadRequest); return }
    ...
    if err := db.DB.Create(&body).Error; err != nil { http.Error(w, "创建失败", http.StatusInternalServerError); return }
    if body.IsDefault {
        db.DB.Model(&models.ProductUnitSpec{}).
            Where("product_id = ? AND id <> ?", body.ProductID, body.ID).
            Update("is_default", false)
    }
    w.Header().Set("Content-Type", "application/json"); json.NewEncoder(w).Encode(body)
}
...
func DeleteProductUnitSpec(w http.ResponseWriter, r *http.Request) {
    ...
    if err := db.DB.Delete(&models.ProductUnitSpec{}, uint(id)).Error; err != nil { http.Error(w, "删除失败", http.StatusInternalServerError); return }
...
func UpsertProductUnitSpecByName(w http.ResponseWriter, r *http.Request) {
    ...
    if err := json.NewDecoder(r.Body).Decode(&body); err != nil { http.Error(w, "参数错误", http.StatusBadRequest); return }
    if strings.TrimSpace(body.ProductName) == "" || strings.TrimSpace(body.Unit) == "" || body.FactorToBase <= 0 { http.Error(w, "参数不完整", http.StatusBadRequest); return }
    ...
    if err := db.DB.Where("name = ?", body.ProductName).First(&prod).Error; err != nil {
        ...
        if err := db.DB.Create(&prod).Error; err != nil { http.Error(w, "创建商品失败", http.StatusInternalServerError); return }
    }
    // upsert spec
    ...
    if err := db.DB.Where("product_id = ? AND unit = ?", prod.ID, body.Unit).First(&spec).Error; err == nil {
        ...
        if body.Kind != "" { spec.Kind = body.Kind }
        spec.IsDefault = body.IsDefault
        if err := db.DB.Save(&spec).Error; err != nil { http.Error(w, "更新失败", http.StatusInternalServerError); return }
        if spec.IsDefault {
            db.DB.Model(&models.ProductUnitSpec{}).
                Where("product_id = ? AND id <> ?", spec.ProductID, spec.ID).
                Update("is_default", false)
        }
        w.Header().Set("Content-Type", "application/json"); json.NewEncoder(w).Encode(spec); return
    }
    ...
    if spec.Kind == "" { spec.Kind = "both" }
    if err := db.DB.Create(&spec).Error; err != nil { http.Error(w, "创建失败", http.StatusInternalServerError); return }
    if spec.IsDefault {
        db.DB.Model(&models.ProductUnitSpec{}).
            Where("product_id = ? AND id <> ?", spec.ProductID, spec.ID).
            Update("is_default", false)
